...
def manage_inventory(initial, commands):
    reject_count = 0
    # commands를 기준으로 요소 순회. for문 사용
    for order in commands:
        if order[0] in list(initial.keys()): # order 항목이 key에 있다면
            cal = initial[order[0]] + order[1]
            if cal >= 0:
                initial[order[0]] = cal # 계산 후 수량이 0이상이면 업데이트
            else:
                reject_count += 1 # 계산 후 수량이 음수면 리젝 카운트
        else: # order 항목이 key에 없다면
            if order[1] >= 0: # 없는 품목이 입고면
                initial[order[0]] = order[1]
            else: # 없는 품목이 출고 신청이면
                initial[order[0]] = 0
                reject_count += 1
    result_list = []
    result_list.append(initial)
    result_list.append(reject_count)
