...
if file_path.exists():  # 파일이 존재할 경우
    ...
    with file_path.open('r', encoding='utf-8') as file:
        data = json.load(file)
        print(type(data))  # 파일을 열고 JSON 데이터를 읽는 코드 (파일 열기, json.load 사용)

    # 3. 어린이 도서가 아닌 책 필터링
    non_children_books = []  # 어린이 도서가 아닌 책을 저장할 리스트 
    # 500파일에는 item이 없기 때문에 in data 다음에 ['item']을 안써도 됨.
    for book in data:
...
# 만약 앞에서 제목만 append했다면 카테고리를 출력할 수 없어서, non_children_books.append(book) 형태로 저장해야 해요.
