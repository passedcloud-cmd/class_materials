...
if file_path.exists():  # 파일이 존재할 경우
    ...
    average_review_ranks = sum(customer_review_ranks) / len(customer_review_ranks) # 평균 리뷰 순위를 계산하는 코드 (sum(customer_review_ranks) / len(customer_review_ranks))

    # 5. 결과 출력
    # 계산된 평균 고객 리뷰 순위를 출력합니다.
    print(f"도서의 평균 고객 리뷰 순위: {average_review_ranks:.2f}")
